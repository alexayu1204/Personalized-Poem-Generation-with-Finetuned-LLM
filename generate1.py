...
def run_generation(model, temp_adapter_dir, prompt):
    ...
    and prompt. Return the raw generated text.
    """
    cmd = [
        "mlx_lm.generate",
        "--model", model,
...
def parse_generation_output(output):
    ...
    if len(parts) >= 3:
        body = parts[1].strip()
        extra = parts[2]
        # Extract token counts with regex.
        prompt_token = None
        generate_token = None
    ...
    # Fallback if the expected markers are not found.
...
def main():
    ...
    parser.add_argument("--user-phase", required=True,
                        help="The user phase (e.g., '小雪'). This will be used as the title and for output filenames.")
    ...
    parser.add_argument("--output-txt", default=None,
                        help="Output TXT filename (default: <user-phase>.txt).")
    parser.add_argument("--output-json", default=None,
                        help="Output JSON filename (default: <user-phase>_metadata.json).")
    ...
    unique_generations = {}  # key: normalized body text, value: metadata dict
    ...
    for epoch, cp_path in checkpoint_list:
        ...
        for idx, prompt in enumerate(prompt_variants, start=1):
            ...
            with tempfile.TemporaryDirectory() as temp_adapter_dir:
                ...
                # Run generation.
                raw_output = run_generation(args.model, temp_adapter_dir, prompt)
